/**
 * Cloudflare Worker: server-side OG/Twitter meta + schema.org injection
 * for /p?id=... product pages, so WhatsApp/Facebook/Twitter crawlers
 * (which don't run JS) see real product data instead of the generic head.
 *
 * Bind this worker to a route covering https://devtem.org/p*
 * Env vars needed (Worker Settings → Variables):
 *   SUPABASE_URL       e.g. https://xxxx.supabase.co
 *   SUPABASE_ANON_KEY   your anon/public key (read-only select is fine)
 *
 * Falls back to the untouched origin HTML on ANY error, missing id,
 * missing product, or non-HTML response — never breaks the page.
 */

export default {
  async fetch(request, env, ctx) {
    const url = new URL(request.url);

    // Only touch the product detail route
    if (url.pathname !== '/p') {
      return fetch(request);
    }

    const productId = url.searchParams.get('id');
    const originResponse = await fetch(request);

    // No id, or origin didn't return HTML -> just pass through
    const contentType = originResponse.headers.get('content-type') || '';
    if (!productId || !contentType.includes('text/html')) {
      return originResponse;
    }

    try {
      const { SUPABASE_URL, SUPABASE_ANON_KEY } = env;

      // Synthetic cache key so repeated crawler re-scrapes (WhatsApp/FB
      // hit shared links again on every re-share) don't hammer Supabase.
      const cache = caches.default;
      const cacheKey = new Request(`https://cache.internal/og-meta/${productId}`);

      let product = null;
      const cached = await cache.match(cacheKey);

      if (cached) {
        product = await cached.json();
      } else {
        const res = await fetch(
          `${SUPABASE_URL}/rest/v1/posts?id=eq.${encodeURIComponent(productId)}` +
            `&select=id,name,description,price,cover,created_at,labels`,
          {
            headers: {
              apikey: SUPABASE_ANON_KEY,
              Authorization: `Bearer ${SUPABASE_ANON_KEY}`,
            },
          }
        );

        if (!res.ok) return originResponse;

        const rows = await res.json();
        product = Array.isArray(rows) ? rows[0] : null;
        if (!product) return originResponse;

        // Cache for 15 minutes; write happens after response is sent.
        const cacheResponse = new Response(JSON.stringify(product), {
          headers: {
            'content-type': 'application/json',
            'cache-control': 'max-age=900',
          },
        });
        ctx.waitUntil(cache.put(cacheKey, cacheResponse));
      }

      // Never expose hidden/private products in previews either
      const visibility = product.labels?.visibility;
      if (visibility === 'hidden' || visibility === 'private') {
        return originResponse;
      }

      const title = `${product.name} — DevTemple`;
      const rawDesc = (product.description || '').replace(/\s+/g, ' ').trim();
      const desc = rawDesc.length > 160 ? rawDesc.slice(0, 157) + '...' : rawDesc;
      const image = product.cover || 'https://devtem.org/assets/images/og.jpg';
      const pageUrl = `https://devtem.org/p?id=${product.id}`;
      const isFree = !product.price || product.price === 0;

      const schema = {
        '@context': 'https://schema.org/',
        '@type': 'Product',
        name: product.name,
        description: desc,
        image,
        offers: {
          '@type': 'Offer',
          url: pageUrl,
          priceCurrency: 'NGN',
          price: isFree ? 0 : product.price,
          availability: 'https://schema.org/InStock',
        },
      };

      const rewriter = new HTMLRewriter()
        .on('title#meta-title', {
          element(el) { el.setInnerContent(title); },
        })
        .on('meta#meta-description', {
          element(el) { el.setAttribute('content', desc); },
        })
        .on('meta#og-title', {
          element(el) { el.setAttribute('content', title); },
        })
        .on('meta#og-description', {
          element(el) { el.setAttribute('content', desc); },
        })
        .on('meta#og-image', {
          element(el) { el.setAttribute('content', image); },
        })
        .on('meta#twitter-title', {
          element(el) { el.setAttribute('content', title); },
        })
        .on('meta#twitter-description', {
          element(el) { el.setAttribute('content', desc); },
        })
        .on('meta#twitter-image', {
          element(el) { el.setAttribute('content', image); },
        })
        .on('link[rel="canonical"]', {
          element(el) { el.setAttribute('href', pageUrl); },
        })
        .on('meta[property="og:url"]', {
          element(el) { el.setAttribute('content', pageUrl); },
        })
        .on('head', {
          element(el) {
            // Add og:url if it doesn't already exist in the source HTML
            el.append(
              `<meta property="og:url" content="${pageUrl}">`,
              { html: true }
            );
            el.append(
              `<script type="application/ld+json">${JSON.stringify(schema)}</script>`,
              { html: true }
            );
          },
        });

      return rewriter.transform(originResponse);
    } catch (err) {
      // Any failure (Supabase down, rate limit, parse error) -> safe fallback
      return originResponse;
    }
  },
};
